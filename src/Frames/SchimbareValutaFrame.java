...
/**
 * Clasa reprezinta meniul de selectie si schimbare a valutei contului bancar
 */

public class SchimbareValutaFrame implements ActionListener{
	...
	JButton transformare = new JButton();
	
	String[] monede = {"Leu", "Euro", "Dolarul american", "Lira Sterina", "Francul Elvetian"};
	...
	SchimbareValutaFrame(ContBancar c){
		...
		convertor.setTitle("Bank App - Schimbare Valuta ");
		...
		transformare.setPreferredSize(new Dimension(200,50));
		transformare.setText("Transforma moneda");
		transformare.setFocusable(false);
		transformare.setFont(new Font("Arial", Font.PLAIN, 20));
		transformare.addActionListener(this);
								
		panouBottom.add(transformare);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == transformare) {
			moneda2 = (String) monedaIn.getSelectedItem();
			String s = "" +(conn.calculareSumaInNouaMoneda(c.getMoneda(), moneda2, c.getSold()) - 0.002 * conn.calculareSumaInNouaMoneda(c.getMoneda(), moneda2, c.getSold()));
			conn.convertireSuma(s, moneda2, c);
			convertor.dispose();
			LoggedInUserFrame loggedIn = new LoggedInUserFrame(c);
		}
